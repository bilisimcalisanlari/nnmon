...
$sqlBusy = "select distinct(metric) from perfdata where host='{$_GET['host']}' and (topic='DISKREADSERV' or topic='DISKSERV') and daytime >now() -interval '12 hours' order by metric";
...
foreach ( $disks as $diskName => $disk )
{
  ...
  foreach ( $disk as $dis )
    ...
    {
      $countDisk = 1;      
      $diskHref .= "<a href=\"diskserv2.php?host={$_GET['host']}&beg={$beg}&end={$end}&diskarg={$argDisk}\">{$argDisk} </a>";
      if ( $first )
      {
	$loopArg = $argDisk;
	$first = 0;
...
{
  $sqlSdisk = "select value,daytime from perfdata where topic='DISKSERV' and metric='{$disk}' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  $result = array();
  foreach ( $conn->query($sqlSdisk) as $row )
    $result[$row['daytime']][0] = $row['value'];
  $sqlSRdisk = "select value,daytime from perfdata where topic='DISKREADSERV' and metric='{$disk}' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  foreach ( $conn->query($sqlSRdisk) as $row )
    $result[$row['daytime']][1] = $row['value'];
  $sqlSWdisk = "select value,daytime from perfdata where topic='DISKWRITESERV' and metric='{$disk}' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  foreach ( $conn->query($sqlSWdisk) as $row )
    $result[$row['daytime']][2] = $row['value'];
  ksort($result);
  $strresult = '';
  foreach ( $result as $daytime => $value )
    $strresult .= $daytime .','.$value[0].','.$value[1].','.$value[2].'\n';
  echo "<br><div style='font-size: 18pt; font-weight: bold; text-align: center;'>{$_GET['host']} Disk {$disk} Service Time</div><br>";
  if ( ! $i )
  {
?>
<div style="margin:15px auto 10px auto;">
  <form name="input" action="diskserv.php" method="get">
...
<script type="text/javascript">
  g = new Dygraph(
    ...
    "Time, Busy, Busy Read, Busy Write %\n" +
    "<?= $strresult ?>"
  );
</script>
