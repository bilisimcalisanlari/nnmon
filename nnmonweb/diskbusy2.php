...
$sqlBusy = "select distinct(metric) from perfdata where host='{$_GET['host']}' and topic='DISKBUSY' and daytime >now() -interval '12 hours' order by metric";
...
foreach ( $disks as $diskName => $disk )
{
  ...
  foreach ( $disk as $dis )
    ...
    {
      $countDisk = 1;      
      $diskHref .= "<a href=\"diskbusy2.php?host={$_GET['host']}&beg={$beg}&end={$end}&diskarg={$argDisk}\">{$argDisk} </a>";
      if ( $first )
      {
	$loopArg = $argDisk;
	$first = 0;
...
<div style="margin:15px auto 10px auto;">
  <form name="input" action="diskbusy2.php" method="get">
