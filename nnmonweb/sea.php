...
$sqlEnt = "select distinct(metric) from perfdata where host='{$_GET['host']}' and topic='SEA' and metric like '%-read-KB/s' and ". $begstr . $endstr . " order by metric";
$ents = array();
foreach ( $conn->query($sqlEnt) as $row )
{
  preg_match( '/(\w+)-read-KB\/s/', $row[0], $uyan );
...
{
  $result = array();
  $sqlRead = "select value,daytime from perfdata where topic='SEA' and metric='{$ent}-read-KB/s' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  foreach ( $conn->query($sqlRead) as $row )
    $result[$row['daytime']][0] = $row['value'];

  $sqlWrite = "select value,daytime from perfdata where topic='SEA' and metric='{$ent}-write-KB/s' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
...
<div style="margin:15px auto 10px auto;">
  <form name="input" action="sea.php" method="get">
