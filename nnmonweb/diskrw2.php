...
$sqlList = "select distinct(metric) from perfdata where host='{$_GET['host']}' and topic='DISKREAD' and daytime >now() -interval '12 hours' order by metric";
...
foreach ( $disks as $diskName => $disk )
{
  ...
  foreach ( $disk as $dis )
    ...
    {
      $countDisk = 1;      
      $diskHref .= "<a href=\"diskrw2.php?host={$_GET['host']}&beg={$beg}&end={$end}&diskarg={$argDisk}\">{$argDisk} </a>";
      if ( $first )
      {
	$loopArg = $argDisk;
	$first = 0;
...
{
  $sqlRead = "select value,daytime from perfdata where topic='DISKREAD' and metric='{$disk}' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  $result = array();
  foreach ( $conn->query($sqlRead) as $row )
    $result[$row['daytime']][0] = $row['value'];
  $sqlWrite = "select value,daytime from perfdata where topic='DISKWRITE' and metric='{$disk}' and host='" . $_GET['host'] ."' and " . $begstr . $endstr;
  ...
  echo "<br><div style='font-size: 18pt; font-weight: bold; text-align: center;'>{$_GET['host']} Disk {$disk} Read / Write</div><br>";
  if ( ! $i )
  {
?>
<div style="margin:15px auto 10px auto;">
  <form name="input" action="diskrw2.php" method="get">
